...
public class NhObbyMaulPureLoadout extends DynamicNhLoadout {
    ...
    public PrayerHandler.PrayerData[] getMeleePrayers() {
        return new PrayerHandler.PrayerData[] {
            PrayerHandler.PrayerData.ULTIMATE_STRENGTH, PrayerHandler.PrayerData.STEEL_SKIN
        };
    }
    ...
    protected void applyRandomization(final RandomizerContext randomizerContext) {
        ...
        // 10% chance to use seers ring instead of berserker ring
        if (randomizerContext.getRandom().nextInt(10) == 1) {
            randomizerContext.swapAll(BERSERKER_RING_I_, SEERS_RING_I_);
        }
        // 10% chance to use regular fury
        if (randomizerContext.getRandom().nextInt(10) == 1) {
            randomizerContext.swapAll(AMULET_OF_BLOOD_FURY, AMULET_OF_FURY);
        }
        // 10% chance to use fire cape instead of infernal
        if (randomizerContext.getRandom().nextInt(10) == 1) {
        ...
        // 10% chance to use granite maul as a KO swap
        if (randomizerContext.getRandom().nextInt(10) == 1) {
        ...
        // 5% chance to vary ranged pot doses
        else if (randomizerContext.getRandom().nextInt(20) == 1) {
